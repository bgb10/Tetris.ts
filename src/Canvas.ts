...
class Canvas2D {
  ...
  public resizeCanvas(): void {
    ...
    let newHeight: number =
      window.innerHeight - 10 >= GAME_CONFIG.CANVAS_MIN_HEIGHT ? window.innerHeight - 10 : GAME_CONFIG.CANVAS_MIN_HEIGHT
    let newWidth: number =
      window.innerWidth >= GAME_CONFIG.CANVAS_MIN_WIDTH ? window.innerWidth : GAME_CONFIG.CANVAS_MIN_WIDTH
  ...
  // lineWidth, strokeStyle, fillStyle 등을 변경한 후 이전의 스타일로 돌려놓기 위해서 save, restore 사용
  // 그림을 바꾸는게 아니라 붓을 바꾸는 것이다!
